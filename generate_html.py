...
def generate_html_from_csv(df):
    ...
    for index, row in df.iterrows():
        try:
            ...
            links_html_parts = []
            if paper_link: links_html_parts.append(f'<a href="{paper_link}" target="_blank">原文</a>')
            if github_link: links_html_parts.append(f'<a href="{github_link}" target="_blank">代码</a>')
...
def main():
    ...
    <!DOCTYPE html><html lang="zh-CN"><head>
        ...
        <style>
            ...
            header {{ text-align: center; margin-bottom: 2em; }} header h1 {{ font-size: 2em; }} header p {{ font-size: 1.2em; color: #57606a; }} header a {{ color: #0969da; text-decoration: none; font-weight: bold; }}
        ...
        <div class="container">
            <header>
                ...
                <div class="header-actions"><a href="{propose_new_url}" class="propose-btn" target="_blank">💡 推荐一篇新论文</a></div>
            </header>
            <table id="paperTable" class="display" style="width:100%">
                <thead><tr>
                    <th style="width: 40%;">论文名称 & 作者</th><th>会议/期刊 & 年份</th><th>形式</th><th>相关链接</th><th>状态 / 操作</th>
                </tr></thead>
        ...
        <script>
            $(document).ready(function() {{ $('#paperTable').DataTable({{ "pageLength": 25, "order": [], "language": {{ "search": "🔍 搜索:", "lengthMenu": "每页显示 _MENU_ 条", "info": "显示第 _START_ 到 _END_ 条，共 _TOTAL_ 条", "infoEmpty": "暂无数据", "infoFiltered": "(从 _MAX_ 条总记录中筛选)", "paginate": {{ "first": "首页", "last": "末页", "next": "下一页", "previous": "上一页" }}, "zeroRecords": "没有找到匹配的记录" }} }}); }});
        </script>
    </body></html>
    """
